            ...
            <form id="payment-form" method="POST" action="{{ route('debts.add-payment', $debt) }}">
                ...
                <div class="mb-3">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" step="0.01" name="amount" id="payment-amount" 
                               class="form-control" max="{{ $debt->remaining_amount }}" required>
                    </div>
                ...
                <div class="mb-3">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="create_sale" 
                               id="create-sale-checkbox" checked>
                        <label class="form-check-label" for="create-sale-checkbox">
                            <strong>Criar venda automaticamente</strong>
                            <div class="small text-muted">Gera uma venda com os produtos desta dívida quando totalmente quitada</div>
                        </label>
                    </div>
                </div>
                @endif
    ...
    <div class="d-flex justify-content-between align-items-start mb-4">
        <div>
            <div class="d-flex align-items-center mb-2">
                <i class="fas {{ $debt->debt_type_icon }} fa-2x me-3 text-{{ $debt->isProductDebt() ? 'primary' : 'success' }}"></i>
                <div>
                    <h1 class="h3 mb-1">Dívida #{{ $debt->id }}</h1>
                    <div class="d-flex align-items-center gap-3">
                        <span class="badge {{ $debt->status_badge }} fs-6">{{ $debt->status_text }}</span>
                        <span class="badge bg-{{ $debt->isProductDebt() ? 'primary' : 'success' }} bg-opacity-10 text-{{ $debt->isProductDebt() ? 'primary' : 'success' }}">
                            {{ $debt->debt_type_text }}
                        </span>
                        @if($debt->is_overdue)
    ...
    <div class="card mb-4">
        <div class="card-body">
            <div class="row g-2">
                ...
                <div class="col-md-3">
                    <button type="button" class="btn btn-success w-100" data-bs-toggle="offcanvas" data-bs-target="#paymentOffcanvas">
                ...
                @if($debt->canBeEdited())
                <div class="col-md-3">
                    <a href="{{ route('debts.index') }}?edit={{ $debt->id }}" class="btn btn-outline-primary w-100">
                ...
                <div class="col-md-3">
                    <button type="button" class="btn btn-outline-danger w-100" onclick="cancelDebt()">
                                ...
                                <tr>
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="me-3">
                                                <i class="fas fa-{{ $item->product->type === 'product' ? 'cube' : 'cog' }} 
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <a href="{{ route('sales.show', $debt->generated_sale_id) }}" 
...
function markAsPaid() {
    ...
    fetch('{{ route("debts.mark-as-paid", $debt) }}', {
        method: 'PATCH',
        headers: {
            'X-Requested-With': 'XMLHttpRequest',
            'Accept': 'application/json',
        ...
        body: JSON.stringify({
            ...
            create_sale: {{ $debt->isProductDebt() ? 'true' : 'false' }}
        })
    })
    .then(response => response.json())
    .then(data => {
...
// Cancelar dívida
function cancelDebt() {
    const message = {{ $debt->isProductDebt() ? '"Tem certeza que deseja cancelar esta dívida? O estoque dos produtos será devolvido."' : '"Tem certeza que deseja cancelar esta dívida?"' }};
    
    if (!confirm(message)) {
        return;
    }
    
    fetch('{{ route("debts.cancel", $debt) }}', {
        method: 'PATCH',
        headers: {
            'X-Requested-With': 'XMLHttpRequest',
            'Accept': 'application/json',
    ...
    .then(data => {
        if (data.success) {
            showToast(data.message, 'success');
            setTimeout(() => window.location.href = '{{ route("debts.index") }}', 1000);
        } else {
...
function createManualSale() {
    ...
    fetch('{{ route("debts.create-manual-sale", $debt) }}', {
        method: 'POST',
        headers: {
            'X-Requested-With': 'XMLHttpRequest',
            'Accept': 'application/json',
